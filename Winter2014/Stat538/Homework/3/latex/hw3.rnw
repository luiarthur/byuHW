...
\begin{document}
\title{Stat538 HW3 - Kaplan Meier Curves}
\author{Arthur Lui}
\maketitle

<<design,echo=F>>=
  source("../../2/R/hw2.R",chdir=T)
...
\wl\noindent
Fishers exact test indicates that there is no association between 
``Treatment Received" and `Age`", ``Initial Stage", or ``Gender".  
So, the data was not stratified.
Using the logrank test, we found that the difference between the
two treatments were \textit{not} significantly different at the 95\% 
significant level. The p-value for the remission duration was 
$\bm{\Sexpr{round(p.remission.logrank,4)}}$. The p-value for the survival
time was $\bm{\Sexpr{round(p.survival.logrank,4)}}$. 
...
\wl\noindent
  ...
  \wl\noindent
  \centering
  \includegraphics{../../2/R/remission.pdf}
...
\begin{figure}  
  ...
  \wl\noindent
  \centering
  \includegraphics{../../2/R/survival.pdf}
...
\wl\noindent  
  \centering
  \includegraphics{../../2/SAS/remission.png}
  \caption{KM curve for remission duration (months). Solid lines are 
           the KM estimates of the suvival function. Dotted lines
...
\begin{figure}  
  \centering
  \includegraphics{../../2/SAS/survival.png}
  \caption{KM curve for survival time (months). Solid lines are 
           the KM estimates of the suvival function. Dotted lines
           are the confidence intervals. Generated with SAS.}
\end{figure} 
...
\section*{Appendix:}
  \subsection*{R Code:}
    \verbatiminput{../../2/R/hw2.R}
    \newpage
  \subsection*{SAS Code:}
    \verbatiminput{../../2/SAS/hw2.sas}
